...
def test(model, dataset_folder, config_file, checkpoint_path):
    import os, sys
    import json
    ...
    image_id = os.listdir(dataset_folder)

    result = {}
    for i in range(len(image_id)):
        image = Image.open(f"{dataset_folder}/{image_id[i]}")
        img, _ = transform(image, None)
        width, height = image.size
        with torch.no_grad():
            # forward pass to get class logits and bounding boxes
            output = model.cuda()(img[None].cuda())
            output = postprocessors['bbox'](output, torch.Tensor([[height, width]]).cuda())[0]
